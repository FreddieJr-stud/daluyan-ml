...
class SOCRealtimePredictor:
    ...
    def update(self, telemetry: dict, current_time: "datetime | None" = None) -> dict | None:
        ...
        Returns:
            ...
            reachable_stations, or None if not enough warmup data.
        """
        ...
        # Need at least 120 ticks for rolling features
        ...
        # Trip progress
        trip_km = telemetry.get("trip_nm", 0) * 1.852
        ...
        soc_diffs = np.diff(soc_arr[-61:]) if len(soc_arr) >= 61 else np.array([0.0])
        soc_rate_std = float(np.std(soc_diffs)) if len(soc_diffs) > 1 else 0.0
        power_std = float(np.std(power_arr[-60:])) if len(power_arr) >= 60 else 0.0
        speed_accel = float(speed_arr[-1] - speed_arr[-30]) if len(speed_arr) >= 30 else 0.0
        ...
        features = {
            ...
            # Rolling windows
            "soc_rate_30s": (soc_arr[-1] - soc_arr[-30]) / 0.5 if len(soc_arr) >= 30 else 0,
            "soc_rate_60s": (soc_arr[-1] - soc_arr[-60]) / 1.0 if len(soc_arr) >= 60 else 0,
            "soc_rate_120s": (soc_arr[-1] - soc_arr[-120]) / 2.0 if len(soc_arr) >= 120 else 0,
            "avg_power_30s": float(power_arr[-30:].mean()),
            "avg_power_60s": float(power_arr[-60:].mean()),
            "avg_speed_30s": float(speed_arr[-30:].mean()),
            "avg_speed_60s": float(speed_arr[-60:].mean()),
            # Route context
        ...
        # Estimate range beyond current trip
