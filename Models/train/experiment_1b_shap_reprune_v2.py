"""SHAP re-pruning v2: 28 candidates (no route prior).

After the first SHAP re-prune showed route_avg_soc_delta (SHAP rank #2)
causes severe overfitting (val-test gap +0.6%), this script excludes it
and tests only the 3 trip-so-far features as new candidates.

28 candidates = 25 production + 3 trip-so-far features.

Usage:
    python -m train.experiment_1b_shap_reprune_v2
"""
...
# 28 candidates: production 25 + 3 trip-so-far (NO route_avg_soc_delta)
...
def main():
    ...
    print("\n" + "=" * 70)
    print("  MODEL 1B: SHAP Re-Pruning v2 (28 candidates, no route prior)")
    print("=" * 70)
    ...
    for f in CANDIDATE_FEATURES:
        assert f in train.columns, f"Missing: {f}"
    print(f"  All {len(CANDIDATE_FEATURES)} candidate features verified.")
    ...
    base_bins = bin_maes(base_errors, prog_test)
    base_val_mae = evaluate(prod_models, val, REALTIME_FEATURES)[0]

    print(f"\n  Production baseline (25 features):")
    print(f"    MAE={base_mae:.4f}%  RMSE={base_rmse:.4f}%  R2={base_r2:.4f}")
    print(f"    Val MAE={base_val_mae:.4f}%  Gap={base_mae - base_val_mae:+.4f}%")
    print_bins(base_errors, prog_test)

    # ── Train on all 28 candidates ──
    print(f"\n  Training on all {len(CANDIDATE_FEATURES)} candidates...")
    t0 = time.time()
    models_28 = train_ensemble(train, val, CANDIDATE_FEATURES, params, n_est)
    print(f"  Done ({time.time()-t0:.1f}s)")

    mae_28, _, r2_28, _, err_28 = evaluate(models_28, test, CANDIDATE_FEATURES)
    val_28 = evaluate(models_28, val, CANDIDATE_FEATURES)[0]
    bins_28 = bin_maes(err_28, prog_test)
    print(f"  All-28: MAE={mae_28:.4f}% ({(mae_28-base_mae)/base_mae*100:+.2f}%)  "
          f"Val={val_28:.4f}%  Gap={mae_28-val_28:+.4f}%  R2={r2_28:.4f}")
    print_bins(err_28, prog_test, base_bins)

    # ── SHAP analysis ──
    print(f"\n  Computing SHAP values (3000 val samples)...")
    n_shap = min(3000, len(val))
    ...
    print(f"  {'Rank':>4s}  {'Feature':<35s}  {'SHAP':>8s}  {'Source':>8s}")
    print(f"  {'-'*4}  {'-'*35}  {'-'*8}  {'-'*8}")
    for i, fi in enumerate(ranked_idx):
        fname = CANDIDATE_FEATURES[fi]
        src = "NEW" if fname not in REALTIME_FEATURES else "prod"
        print(f"  {i+1:4d}  {fname:<35s}  {mean_shap[fi]:8.4f}  {src:>8s}")

    # ── Pruning sweep ──
    # Test a wide range to find the sweet spot
    n_values = [20, 22, 24, 25, 26, 27, 28]
    results = []
    ...
    for n_keep in n_values:
        ...
        if new_in:
            print(f"    Added:   {', '.join(new_in)}")
        if old_out:
            print(f"    Dropped: {', '.join(old_out)}")
        ...
        bins = bin_maes(errors, prog_test)
        ...
        print_bins(errors, prog_test, base_bins)
    ...
    print(f"  {25:3d}  {base_mae:7.4f}  {'base':>8s}  {base_val_mae:7.4f}  "
          f"{base_mae-base_val_mae:+7.4f}  "
    ...
    if passed_results:
        ...
        pct_025 = (best["bins"].get("0-25%",0)-base_bins.get("0-25%",0))/base_bins.get("0-25%",1)*100
        print(f"\n  RECOMMENDATION: Top-{best['n']} features")
        print(f"    MAE: {best['mae']:.4f}% ({best['delta']:+.2f}% vs production)")
        print(f"    Val-test gap: {best['gap']:+.4f}%")
        print(f"    0-25% bin: {best['bins'].get('0-25%',0):.4f}% ({pct_025:+.1f}% vs prod)")
        if best["new_in"]:
        ...
        closest = min(results, key=lambda r: r["mae"])
        print(f"  Closest: N={closest['n']}, MAE={closest['mae']:.4f}% ({closest['delta']:+.2f}%)")
